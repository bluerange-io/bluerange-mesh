...
int SEGGER_RTT_Write(unsigned BufferIndex, const char* pBuffer, unsigned NumBytes) {
  ...
  //This can be used to enable a rolling log that helps when debugging problems while a debugger was not attached
  if(SEGGER_RTT_USE_ROLLING_LOG)
  {
	  //Check how much free space there is left
	  int freeSpace =  _SEGGER_RTT.aUp[BufferIndex].RdOff - _SEGGER_RTT.aUp[BufferIndex].WrOff - 1;
	  if (freeSpace < 0) {
		  freeSpace += _SEGGER_RTT.aUp[BufferIndex].SizeOfBuffer;
	  }

	  //If it is not enough to save our buffer, release some space
	  if((unsigned int)freeSpace < NumBytes){
		  _SEGGER_RTT.aUp[BufferIndex].RdOff += NumBytes;

		  if(_SEGGER_RTT.aUp[BufferIndex].RdOff > _SEGGER_RTT.aUp[BufferIndex].SizeOfBuffer){
			  _SEGGER_RTT.aUp[BufferIndex].RdOff -= _SEGGER_RTT.aUp[BufferIndex].SizeOfBuffer;
		  }

		  //Write some stuff at the beginning of the truncated log so that we can see the truncation
		  MEMCPY(_SEGGER_RTT.aUp[BufferIndex].pBuffer + _SEGGER_RTT.aUp[BufferIndex].RdOff, "?", 1);
	  }
