...
#define MESH_ACCESS_MODULE_CONFIG_VERSION 1
...
MeshAccessModule::MeshAccessModule()
	: Module(moduleID::MESH_ACCESS_MODULE_ID, "ma")
{
	moduleVersion = MESH_ACCESS_MODULE_CONFIG_VERSION;
...
void MeshAccessModule::ResetToDefaultConfiguration()
{
	...
	configuration.allowInboundConnections = true;
	configuration.enableAdvertising = true;
	configuration.disableIfInMesh = false;
...
void MeshAccessModule::ConfigurationLoadedHandler(ModuleConfiguration* migratableConfig, u16 migratableConfigLength)
{
	...
	if(configuration.moduleActive){
		...
		if(configuration.allowInboundConnections){
			RegisterGattService();
...
void MeshAccessModule::RegisterGattService()
{
	...
	memset(&rxAttributeMetadata, 0, sizeof(ble_gatts_attr_md_t));
	...
	memset(&rxCharacteristicMetadata, 0, sizeof(ble_gatts_char_md_t));
	...
	memset(&rxAttribute, 0, sizeof(ble_gatts_attr_t));
	...
	memset(&txCharacteristicMetadata, 0, sizeof(ble_gatts_char_md_t));
	...
	memset(&txAttributeMetadata, 0, sizeof(ble_gatts_attr_md_t));
	...
	memset(&txAttribute, 0, sizeof(ble_gatts_attr_t));
...
{
	if(!configuration.enableAdvertising){
	...
	serviceData->isEnrolled = GS->node->configuration.enrollmentState;
	serviceData->serialIndex = RamConfig->serialNumberIndex;
	...
	char cbuffer[100];
	GS->logger->convertBufferToHexString(buffer, length, cbuffer, sizeof(cbuffer));
...
void MeshAccessModule::AddModuleIdToAdvertise(moduleID moduleId)
{
	...
	for(u32 i=0; i<3; i++){
		if(moduleIdsToAdvertise[i] == (u8)moduleId) return;
	}
	//Insert into empty slot
	for(u32 i=0; i<3; i++){
		if(moduleIdsToAdvertise[i] == 0){
			moduleIdsToAdvertise[i] = (u8)moduleId;
			BroadcastMeshAccessPacket();
			return;
		}
	}
...
void MeshAccessModule::ReceivedMeshAccessConnectionStateMessage(connPacketModule* packet, u16 packetLength) const
{
	...
	logjson("MAMOD", "{\"nodeId\":%u,\"type\":\"ma_conn_state\",\"module\":%u,", packet->header.sender, packet->moduleId);
	logjson("MAMOD",  "\"requestHandle\":%u,\"vPartnerId\":%u,\"state\":%u}" SEP, packet->requestHandle, message->vPartnerId, message->state);
}

MeshAccessAuthorization MeshAccessModule::CheckMeshAccessPacketAuthorization(BaseConnectionSendData* sendData, u8* data, u32 fmKeyId)
{
	connPacketHeader* packet = (connPacketHeader*)data;

	//We must always whitelist handshake packets for the MeshAccess Connection
	if(packet->messageType >= MESSAGE_TYPE_ENCRYPT_CUSTOM_START
			&& packet->messageType <= MESSAGE_TYPE_ENCRYPT_CUSTOM_DONE)
	{
		return MeshAccessAuthorization::MA_AUTH_WHITELIST;
	}
	...
	if(fmKeyId == FM_KEY_ID_NONE){
			return MeshAccessAuthorization::MA_AUTH_LOCAL_ONLY;
		...
		return MeshAccessAuthorization::MA_AUTH_UNDETERMINED;
	} else if(fmKeyId == FM_KEY_ID_ORGANIZATION){
		return MeshAccessAuthorization::MA_AUTH_UNDETERMINED;
	} else {
		return MeshAccessAuthorization::MA_AUTH_BLACKLIST;
	}
}

MeshAccessAuthorization MeshAccessModule::CheckAuthorizationForAll(BaseConnectionSendData* sendData, u8* data, u32 fmKeyId) const
{
	//Check if our partner is authorized to send this packet, so we ask
	//all of our modules if this message is whitelisted or not
	MeshAccessAuthorization auth = MeshAccessAuthorization::MA_AUTH_UNDETERMINED;
	for (int i = 0; i < MAX_MODULE_COUNT; i++){
		if (GS->activeModules[i] != nullptr && GS->activeModules[i]->configurationPointer->moduleActive){
			MeshAccessAuthorization newAuth = GS->activeModules[i]->CheckMeshAccessPacketAuthorization(sendData, data, fmKeyId);
			if(newAuth > auth){
				auth = newAuth;
			}
		}
	}
	logt("MACONN", "Message auth is %u", auth);
...
void MeshAccessModule::MeshConnectionChangedHandler(MeshConnection& connection)
{
	...
	if(configuration.disableIfInMesh)
	{
...
bool MeshAccessModule::TerminalCommandHandler(char* commandArgs[], u8 commandArgsSize)
{
	...
	if(TERMARGS(0, "maconn"))
	{
		...
		GS->logger->parseHexStringToBuffer(commandArgs[1], addr.addr, 6);
	...
	else if(TERMARGS(0, "malog"))	//jstodo: maybe unused? At least in the simulator, logNearby is never used. If it were, it would print an ERROR in BleEventHandler
	{
		logNearby = !logNearby;
	...
	if(commandArgsSize >= 4 && TERMARGS(0, "action") && TERMARGS(2, moduleName))
	{
		...
		if(TERMARGS(3,"connect") && commandArgsSize >= 5)
		{
			...
			GS->logger->parseHexStringToBuffer(commandArgs[4], data.targetAddress.addr, 6);
			Utility::swapBytes(data.targetAddress.addr, 6);

			if(commandArgsSize > 5) data.fmKeyId = atoi(commandArgs[5]);
			if(commandArgsSize > 6){
				GS->logger->parseHexStringToBuffer(commandArgs[6], data.key.getRaw(), 16);
			} else {
...
void MeshAccessModule::BleEventHandler(const ble_evt_t& bleEvent)
{
	if(logNearby){
		...
		if (
				bleEvent.evt.gap_evt.params.adv_report.dlen >= SIZEOF_ADV_STRUCTURE_MESH_ACCESS_SERVICE_DATA
			...
			logt("ERROR", "Serial %s, Addr %02X:%02X:%02X:%02X:%02X:%02X, networkId %u, enrolled %u, connectable %u, rssi %d",
					serialNumber,
					bleEvent.evt.gap_evt.params.adv_report.peer_addr.addr[5],
					bleEvent.evt.gap_evt.params.adv_report.peer_addr.addr[4],
					bleEvent.evt.gap_evt.params.adv_report.peer_addr.addr[3],
					bleEvent.evt.gap_evt.params.adv_report.peer_addr.addr[2],
					bleEvent.evt.gap_evt.params.adv_report.peer_addr.addr[1],
					bleEvent.evt.gap_evt.params.adv_report.peer_addr.addr[0],
					maPacket->networkId,
					maPacket->isEnrolled,
					bleEvent.evt.gap_evt.params.adv_report.type == 0,
