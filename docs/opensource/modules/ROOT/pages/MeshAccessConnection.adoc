...
== Purpose
The MeshAccessConnection works closely with the xref:MeshAccessModule.adoc[MeshAccessModule] to provide a simple proxy for connecting to the mesh network from a device that does not need to implement the BlueRange Mesh algorithm.
