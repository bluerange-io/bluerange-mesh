...
----
//Establish a connection to another device using a MeshAccessConnection
action [nodeId] ma serial_connect [serial number] [keyId] [key] {nodeId_of_partner_after_connect = 0} {initial_keep_alive = 0} {requestHandle=0}
...
If no `nodeId_of_partner_after_connect` is given or it is set to 0, a virtual partner id is assigned to the connection. If it is given, the nodeId must come of the range of organization wide unique nodeIds.

If no `initial_keep_alive` is given or it is set to 0, the connection will stay open indefinitely. **NOTE** This variable is merely a suggestion to the node. There are a lot of cases where a connection can also be prematurely disconnected.
