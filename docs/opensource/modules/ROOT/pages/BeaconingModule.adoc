...
== Purpose
The _BeaconingModule_ with _ModuleId_ 1 allows the user to configure a number of advertising/beaconing messages to be sent periodically. It is possible to send iBeacon messages, EddyStone messages and other custom advertising messages without the need to update the firmware. Using BlueRange Mesh in conjunction with a MeshGateway, a fully managed beacon infrastructure can be established.
