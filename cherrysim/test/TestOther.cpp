...
TEST(TestOther, TestJsonConfigSerialization)
{
    ...
    simConfig->disableNonCriticalExceptions = true;
    new (&simConfig->floorplanImage) std::string;
    simConfig->floorplanImage = "bla.png";
    simConfig->ignoreDeviceJsonEnrollments = true;
    simConfig->webServerPort = 1234;
    simConfig->socketServerPort = 4567;
...
#define IsInSTLRange(x) (byteIndex >= myOffsetOf(simConfig, x) && byteIndex < myOffsetOf(simConfig, x) + sizeof(SimConfiguration::x))
        ...
        if(IsInSTLRange(siteJsonPath)
            ...
            || IsInSTLRange(storeFlashToFile)
            || IsInSTLRange(floorplanImage)) continue;
#undef IsInSTLRange
    ...
    ASSERT_EQ(copy.disableNonCriticalExceptions, true);
    ASSERT_EQ(copy.floorplanImage, "bla.png");
    ASSERT_EQ(copy.ignoreDeviceJsonEnrollments, true);
    ASSERT_EQ(copy.webServerPort, 1234);
    ASSERT_EQ(copy.socketServerPort, 4567);
    ...
    simConfig->floorplanImage.~basic_string();
}
